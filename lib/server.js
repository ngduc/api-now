module.exports = function(options) {
  ...
  if (options.file) {
    ...
    defaultMsgStr = ' - default CRUD routes: /users /posts - other routes: /echo /file /login';
  }
  ...
  server.get('/images/random', (req, res) => {
    const { path } = req.query;
  ...
  server.get('/avatars/random', (req, res) => {
    // const { path } = req.query;
    const fullPath = path.join(__dirname, '../data/avatars');
    utils.getRandomFileFromDir(fullPath, filePath => {
      res.sendFile(filePath);
    });
  });
